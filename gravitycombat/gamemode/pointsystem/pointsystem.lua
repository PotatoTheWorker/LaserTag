    ...
    if (strText == "!menu") then
        ...
        tags.DoClick = function()
            ...
            tagsmenu.Paint = function(s, w, h)
                ...
                surface.DrawRect(0, 0, w, h)
                surface.SetDrawColor(white)
                draw.SimpleText("Noobie", "BasicHudFont", 50, 50, white, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
                draw.SimpleText("Unlocked by joining the server for the first time.", "TinyHudFont", 150, 56, white, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
                if (ply:GetNWBool("noobieunlocked") == true) then
                    draw.SimpleText("Unlocked", "TinyHudFont", 575, 56, white, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
                else
                    draw.SimpleText("Locked", "TinyHudFont", 575, 56, white, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
                end
            end

            local equipnoobietag = vgui.Create("DButton", tagsmenu)
            equipnoobietag:SetPos(675, 50)
            equipnoobietag:SetSize(75, 30)
            equipnoobietag:SetText("")
            equipnoobietag:SetColor(white)

            equipnoobietag.Paint = function(panel, w, h)
            ...
            equipnoobietag.DoClick = function()
                equipid = 1
                sendToServer()
            end
